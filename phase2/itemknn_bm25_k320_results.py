...
from phase2.run_itemknn import main as run_itemknn_main
...
def combine_results(split: str) -> pd.DataFrame:
    model_path = RESULTS_DIR / f"itemknn_bm25_k320_{split}.csv"
    combined = pd.concat(
        [load_baseline_results(split), pd.read_csv(model_path)],
        ignore_index=True,
    )
    output_path = RESULTS_DIR / f"itemknn_bm25_k320_comparison_{split}.csv"
...
print()
print(f"Saved validation comparison to: {RESULTS_DIR / 'itemknn_bm25_k320_comparison_val.csv'}")
print(f"Saved test comparison to: {RESULTS_DIR / 'itemknn_bm25_k320_comparison_test.csv'}")
